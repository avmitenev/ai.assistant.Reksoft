...
st.set_page_config(page_title="Reksoft's Assistant")

def show_ui(qa, prompt_to_user="Как я могу Вам помочь?"):
    """
...
def reset(prompt_to_user="Как я могу Вам помочь?"):
    session_id = get_script_run_ctx().session_id
...
def run():
    ...
    if ready:
        ...
        # Chat Interface
        st.title("Reksoft's Assistant")
        st.caption("Приложение для работы с искусственным интеллектом обучалось на основе общедоступных документов с веб-сайта организации в течение трех дней. Стоит отметить, что точность его ответов может быть повышена за счет доступа к дополнительным материалам.")
        st.subheader("Узнайте больше о спектре услуг Reksoft:")
        show_ui(st.session_state['chain'], "Как я могу Вам помочь?")
        st.button("Сбросить историю", on_click=reset)
